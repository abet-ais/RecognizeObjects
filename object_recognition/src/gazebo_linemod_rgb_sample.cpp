...
void callback(const sensor_msgs::Image::ConstPtr& rgb_image,
  ...
  // if (mouse_event_flag == CV_EVENT_FLAG_RBUTTON) {
    ...
    cv::imshow("canny_mask", mask);
    // Extract template
    std::string class_id = cv::format("%d", num_classes);
    cv::Rect bb;
  ...
  cv::rectangle(display2, pt1, pt2, CV_RGB(0,0,0), 3);
  cv::rectangle(display2, pt1, pt2, CV_RGB(255,255,0), 1);
...
int main(int argc, char** argv){
  ...
  if (argc == 1){
    filename = "linemod_templates_2d.yml";
