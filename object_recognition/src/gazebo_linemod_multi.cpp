...
void gazeboPoseCb(const geometry_msgs::Vector3 pose){
  gazebo_pose = pose;
}

void quaternionGazeboPoseCb(const geometry_msgs::Quaternion quat_pose){
  quat_gazebo_pose = quat_pose;
}
...
int main(int argc, char** argv){
  ...
  Mouse::start("color");
  if (argc == 1){
    filename = "linemod_templates_z40_gear.yml";
  ...
  ros::Subscriber gazebo_pose_sub = nh.subscribe("gazebo_pose", 1, gazeboPoseCb);
  ros::Subscriber quaternion_pose_sub = nh.subscribe("quaternion_gazebo_pose", 1, quaternionGazeboPoseCb);
