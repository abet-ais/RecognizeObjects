...
void callback(const sensor_msgs::Image::ConstPtr& rgb_image,
  ...
  if ( (mouse_event_flag == CV_EVENT_FLAG_RBUTTON || mouse_event_sw == true) && num_classes <= SCALE_CHANGE) {
    ...
    if(switch_client.call(sw_srv)){
      ...
      x_pix = (int)(color.cols * 0.5);
      y_pix = (int)(color.rows * 0.5);
      std::cout <<" check:"<<check<<" x_p:"<<x_pix<<" y_p:"<<y_pix << std::endl;
      if(sw_srv.response.check == 10000){
    ...
    std::string posture = cv::format(",%f,%f,%f,%f", qx, qy, qz, qw);
    //std::cout << "posture = "<< posture << std::endl;

    cv::Rect bb;
    int template_id;
    if(num_classes > 0){
      // template_id = detector->addTemplate(sources, class_id+",0", mask, &bb);
      template_id = detector->addTemplate(sources, class_id+posture, mask, &bb);
      if(template_id != -1){
...
int main(int argc, char** argv){
  ...
  boost::shared_ptr<message_filters::Synchronizer<SyncPolicy> >sync_ = boost::make_shared<message_filters::Synchronizer<SyncPolicy> >(50);
