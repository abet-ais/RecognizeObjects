...
std::string template_path = "/home/ais/ais-project/catkin_ws/src/devel/abe/abe_linemod_pkg/";
              ...
              const sensor_msgs::CameraInfo::ConstPtr& depth_camera_info)
{
  ...
  // cv::Mat imput_color = cv_rgb->image;
  cv::Mat color = cv_rgb->image;
  // cv::Rect rect(448, 156, 1024, 768);
  // cv::Rect rect(640, 300, 640, 480);
  // cv::Mat color(imput_color, rect);
  // cv::imshow("hoge",color);
  ...
  cv_bridge::CvImagePtr cv_depth = cv_bridge::toCvCopy(
    depth_image, sensor_msgs::image_encodings::TYPE_16UC1);
  //cv::Mat depth = 1000 * cv_rgb->image;
  ...
  if (!learn_online)
  {
    ...
    if (event == CV_EVENT_RBUTTONDOWN)
    {
      ...
      cv::rectangle(mask, cv::Point(0,0), cv::Point(640,Mouse::y()-roi_size.height/2), cv::Scalar(0,0,0), -1, CV_AA);
      cv::rectangle(mask, cv::Point(0,Mouse::y()+roi_size.height/2), cv::Point(640,480), cv::Scalar(0,0,0), -1, CV_AA);
      cv::rectangle(mask, cv::Point(0,0), cv::Point(Mouse::x()-roi_size.width/2,480), cv::Scalar(0,0,0), -1, CV_AA);
      cv::rectangle(mask, cv::Point(Mouse::x()+roi_size.width/2,0), cv::Point(640,480), cv::Scalar(0,0,0), -1, CV_AA);
  ...
  /*
  int num =2;
  if((int)matches.size() > num){
    matching_threshold = matching_threshold + 1.0;
  }
  if((int)matches.size() < num && matching_threshold > 60){
    matching_threshold = matching_threshold - 1.0;
  }
  */
  ...
  for (int i = 0; (i < (int)matches.size()) && (classes_visited < num_classes); ++i)
  {
    ...
    if (visited.insert(m.class_id).second){
      ...
      if (show_match_result){
        // printf("Similarity:%5.1f%%; x:%3d; y:%3d; %s; %d; %s\n",
               // m.similarity, m.x, m.y, parts_name.c_str(), deg, m.class_id.c_str());
...
int main(int argc, char** argv)
{
  ...
  sub_rgb_image.subscribe(nh, "/camera/rgb/image_rect_color", 1);
  sub_depth_image.subscribe(nh, "/camera/depth/image_rect_raw", 1);
  sub_rgb_camera_info.subscribe(nh, "/camera/rgb/camera_info", 1);
  sub_depth_camera_info.subscribe(nh, "/camera/depth/camera_info", 1);
  // sub_rgb_image.subscribe(nh, "/camera/color/image_raw", 1);
  // sub_depth_image.subscribe(nh, "/camera/depth/image_raw", 1);
  // sub_rgb_camera_info.subscribe(nh, "/camera/color/camera_info", 1);
  // sub_depth_camera_info.subscribe(nh, "/camera/depth/camera_info", 1);
